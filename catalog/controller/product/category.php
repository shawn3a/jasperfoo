		...
		if ($category_info) {
			...
			foreach ($results as $result) {
				...
				if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
					$priceArray = explode('.', $price = $this->currency->format($this->tax->calculate($result['price'], $result['tax_class_id'], $this->config->get('config_tax'))));
                    $price = $priceArray[0];
				} else {
					$price = false;
				}
				...
				$data['products'][] = array(
					...
					'thumb'       => $image,
					'name'        => $result['name'],
					'description' => $this->state === 'sold' ? strip_tags(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8')) : utf8_substr(strip_tags(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8')), 0, 200) . '..',
					'price'       => $price,
					'special'     => $special,
					'tax'         => $tax,
